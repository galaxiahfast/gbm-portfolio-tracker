...
SNAPSHOT_VERSION = "fundamental-news-v1"
...
def snapshot_from_payload(payload_json: str) -> FundamentalNewsSnapshot:
    ...
    return FundamentalNewsSnapshot(
        ...
        news=tuple(
            NewsItem(**{**item, "topics": tuple(item.get("topics", []))})
...
def build_fundamental_snapshot(
    ...
    *,
    ...
    metrics: dict[str, float | str | None] = {
        ...
        "free_cash_flow": _first_number(source, "freeCashflow") or quarterly_fcf,
    ...
    for item in news:
        ...
        weight = math.exp(-math.log(2) * age_hours / 72) * (1.25 if item.topics else 1.0)
    ...
    earnings_close = any(
        ...
        and timedelta(0) <= (_parse_datetime(item.event_at) - observed) <= timedelta(days=3)
        for item in events
        if _parse_datetime(item.event_at)
    )
    event_points = -2.0 if earnings_close else 0.0
    if earnings_close:
        reasons.append("Resultados en ≤3 días: riesgo de gap y ejecución bloqueada de forma preventiva.")
    ...
    veto_scope = "ALL" if earnings_close else "LONG" if severe_negative else "SHORT" if severe_positive else "NONE"
